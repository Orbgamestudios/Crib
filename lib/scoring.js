...
function scoreBreakdown(hand, starter, isCrib) {
  ...
  while (r <= 13) {
    ...
    let len = 0, mult = 1;
    while (r <= 13 && counts[r]) { len++; mult *= counts[r]; r++; }
  ...
  if (hand.length >= 4 && hand.every(c => c.suit === hand[0].suit)) {
    if (isCrib) {
      if (starter.suit === hand[0].suit) flush = hand.length + 1;
    } else {
      flush = hand.length + (starter.suit === hand[0].suit ? 1 : 0);
    }
  }
...
function pegEvents(stack, count) {
  const events = [];
...
module.exports = { scoreBreakdown, pegEvents };
