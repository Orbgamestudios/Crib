import { cardName, makeCard } from './cards.js';
...
function shopCardItem() {
  ...
  return {
    ...
    suit: c.suit,
    name: cardName(c),
    desc: 'Add this exact playing card permanently to your deck.',
    cost: 3,
    sold: false,
  };
}
...
function wildcardItem(player, excludeJokers) {
  ...
  if (roll === 0) {
    const joker = pick(JOKERS, 1, excludeJokers)[0];
    if (joker) return shopItem(joker, 'joker');
  }
  if (roll === 1) return shopItem(TAROTS[Math.floor(Math.random() * TAROTS.length)], 'tarot');
  return shopCardItem();
}
