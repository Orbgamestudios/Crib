...
export const PACKS = [
  ...
  { id: 'ultra', name: 'Ultra Pack', desc: 'Pick 1 of 3 jokers with much higher rare and ultra odds, plus better stamp odds.', cost: 8, kind: 'pack' },
...
export function buildScoreOrdered(bd, jokerIds, kind, cards, ctx) {
  ...
  if (bd.fifteens) add(`Fifteens ×${bd.fifteens}`, bd.fifteens * fifteenVal);
  ...
  for (const raw of effectiveJokerIds(jokerIds)) {
    ...
    if (kind === 'hand' && joker?.stamp === 'blue') add('Blue Stamp' + J, 2);
