...
export const JOKERS = [
  ...
  { id: 'steel_crib', name: 'Steel Crib', desc: 'Your crib scores double.', cost: 8, mods: { cribMult: 2 } },
  { id: 'overclock', name: 'Overclock', desc: 'All your pegging Mult is tripled.', cost: 8, mods: { pegMult: 3 } },
...
const RARE_IDS = new Set([
  ...
  'rocket', 'coin_clip', 'his_majesty', 'run_baron', 'flush_broker', 'crib_diviner',
]);
const ULTRA_IDS = new Set([
  'blueprint', 'crib_copier', 'obelisk', 'the_duo', 'holo_fifteen', 'steel_crib', 'overclock', 'card_smith',
]);
...
export function aggregateMods(jokerIds) {
  const m = {
    ...
    cribTenTarot: 0, freeStandardPack: 0,
  };
...
function shopItem(def, kind) {
  const item = { kind, id: def.id, name: def.name, desc: def.desc, cost: def.cost, sold: false };
  if (kind === 'joker') item.rarity = def.rarity;
  return item;
}
...
function wildcardItem(player, excludeJokers) {
  ...
  return shopItem(TAROTS[Math.floor(Math.random() * TAROTS.length)], 'tarot');
}
...
export function openPack(type, player) {
  if (type === 'buffoon') {
    return pickJokers(3, player.jokers).map(j => ({ kind: 'joker', id: j.id, name: j.name, desc: j.desc, rarity: j.rarity }));
  }
  if (type === 'arcana') {
