...
export const TAROTS = [
  ...
  { id: 'azure_seal', name: 'Azure Seal', desc: 'Add a Blue Stamp to one owned joker.', cost: 5, targets: 0, jokerStamp: 'blue', rare: true },
];
...
export const STAMPS = {
  ...
  blue: { name: 'Blue Stamp', desc: 'Adds +1 joker slot while owned.', chance: 0.015 },
  red: { name: 'Red Stamp', desc: '+1 Mult during scoring.', chance: 0.03 },
  green: { name: 'Green Stamp', desc: '+2 hand points during scoring.', chance: 0.03 },
};
...
export function normalizeJoker(joker) {
  ...
  if (joker && joker.id) return { id: joker.id, ...(joker.stamp ? { stamp: joker.stamp } : {}) };
  return null;
}
...
export function jokerCapacity(jokers, base = 5) {
  return base + (jokers || []).filter(j => normalizeJoker(j)?.stamp === 'blue').length;
}
...
export function aggregateMods(jokerIds) {
  ...
  for (const raw of jokerIds || []) {
    ...
    else if (j.stamp === 'green') m.stampGreenPoints += 2;
    else if (j.stamp === 'purple') m.stampPurpleCrib = true;
    else if (j.stamp === 'blue') m.stampBlueSlots += 1;
  }
...
export function buildScore(bd, mods, kind, cards, ctx) {
  ...
  if (kind === 'hand' && mods.stampGreenPoints) add('Green Stamp' + J, mods.stampGreenPoints);
