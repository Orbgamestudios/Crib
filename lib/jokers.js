...
export const JOKERS = [
  ...
  { id: 'crib_battery', name: 'Crib Battery', desc: 'Your crib scores with x2 Mult instead of plain points.', cost: 7, mods: { cribScoreMult: 2 } },
...
export function aggregateMods(jokerIds) {
  ...
  for (const id of effectiveJokerIds(jokerIds)) {
    ...
    for (const [k, v] of Object.entries(def.mods)) {
      ...
      else if (k === 'cribPairMult' || k === 'cribFifteenMult') m[k] += v;
