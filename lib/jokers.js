...
const JOKERS_BY_ID = Object.fromEntries(JOKERS.map(j => [j.id, j]));
const TAROTS_BY_ID = Object.fromEntries(TAROTS.map(t => [t.id, t]));

const VAL_KEYS = ['fifteenVal', 'pairVal', 'nobsVal', 'goVal', 'thirtyOneVal', 'heelsVal'];

function aggregateMods(jokerIds) {
  const m = {
...
function applyMods(bd, mods, isCrib, handCards) {
  let total =
...
module.exports = { JOKERS, TAROTS, JOKERS_BY_ID, TAROTS_BY_ID, aggregateMods, applyMods, makeShopOffer };
