...
export class Game {
  ...
  doScoring() {
    ...
    const cribBonusMult = this.mode === 'board' ? 0 : cribBonuses.reduce((sum, n) => sum + n, 0);
    const cribSuitCount = new Set(this.crib.map(c => c.suit)).size;
    const cribSuitFactor = this.mode !== 'board' && dmods.cribSuitMult !== 1 ? Math.pow(dmods.cribSuitMult, cribSuitCount) : 1;
    const cribMult = this.mode === 'board' ? 1 : Math.round((BASE_MULT + dealer.dealPegMult + cribBonusMult) * cribSuitFactor * 100) / 100;
