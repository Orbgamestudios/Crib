...
export class Game {
  ...
  constructor(seats, hooks, opts = {}) {
    ...
    this.mode = opts.mode === 'board' ? 'board' : 'blind';
  ...
  static fromSnapshot(snapshot, hooks) {
    ...
    game.mode = game.mode === 'board' ? 'board' : 'blind';
  ...
  startDeal() {
    ...
    for (const p of this.players) {
      if (p.active) {
        ...
        if (this.mode === 'board') pile.splice(0, this.cardsEach);
        p.drawPile = this.mode === 'board' ? [] : pile.slice(this.cardsEach);
      } else {
        p.hand = []; p.drawPile = [];
      }
  ...
  doScoring() {
    ...
    for (let i = 1; i <= n; i++) {
      ...
      const { total: points, lines } = this.mode === 'board'
        ? buildScore(bd, mods, 'hand', p.kept, { starter, coins: p.coins })
        : buildScoreOrdered(bd, p.jokers, 'hand', p.kept, { starter, coins: p.coins });
      handBlocks.push({
        kind: 'hand', player: p, seat: p.seat, name: p.name,
        cards: p.kept.slice(), lines, points, mult: this.mode === 'board' ? 1 : BASE_MULT + p.dealPegMult,
    ...
    const cribRes = this.mode === 'board'
      ? buildScore(cribBd, dmods, 'crib', this.crib, { starter, coins: dealer.coins })
      : buildScoreOrdered(cribBd, dealer.jokers, 'crib', this.crib, { starter, coins: dealer.coins });
    const cribMultRes = this.mode === 'board'
      ? { mult: 1, lines: [] }
      : this.scoreCribMultOrdered(dealer, cribBd);
    cribRes.lines.push(...cribMultRes.lines);
    const cribMult = cribMultRes.mult;
    const cribBlock = {
    ...
    const results = [];
    const finalize = b => {
      b.total = this.mode === 'board' ? b.points : b.points * b.mult;
