...
const PER_CRIB_YIELD = 6;   // crib scores its points only (no Mult)
...
export function computeBlind(round, playerCount, dealsInRound) {
  ...
  const mult = 0.16 * Math.pow(1.33, round - 1) * HARDER;
...
export class Game {
  ...
  discard(p, cardIds) {
    ...
    if (copies && cribCards[0]) {
      for (let i = 0; i < copies; i++) this.crib.push(makeCard(cribCards[0].rank, cribCards[0].suit));
      this.log(`${p.name}'s Crib Copier copied ${cardName(cribCards[0])} into the crib`);
    }
  ...
  doScoring() {
    ...
    // the crib scores its points straight — no Mult applied
    const cribBlock = {
      kind: 'crib', player: dealer, seat: dealer.seat, name: dealer.name,
      cards: this.crib.slice(), lines: cribRes.lines, points: cribRes.total, mult: 1, noMult: true,
    };
