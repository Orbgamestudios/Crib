...
export class Game {
  ...
  playCard(p, cardId) {
    ...
    let pegMult = mods.pegMult;
    if (pts > 0 && mods.pegHighCountMult !== 1 && this.pegCount >= 15) {
      pegMult *= mods.pegHighCountMult;
      this.log('  Counter King [Joker]');
    }
    if (pts > 0 && mods.pegAfterTwoMult !== 1 && p.pegPlays >= 2) {
      pegMult *= mods.pegAfterTwoMult;
      this.log('  Overclock [Joker]');
    }
    p.pegPlays++;
    pts *= pegMult;
  ...
  doScoring() {
    ...
    const cribMult = BASE_MULT + dealer.dealPegMult + cribBonusMult;
