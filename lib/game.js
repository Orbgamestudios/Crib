...
export class Game {
  ...
  scorePegHandPoints(p, card, events) {
    if (this.mode === 'board' || !events.length) return 0;
    let total = 0;
    for (const raw of effectiveJokerIds(p.jokers)) {
      ...
      if (base <= 0) continue;
      const gain = base + (owned.stamp === 'blue' ? 2 : 0);
      total += gain;
      p.dealHandBonusLines.push({ label: `${def.name} [Joker]`, pts: gain });
      this.log(`  ${def.name} +${gain} Hand Points [Joker]`);
    }
