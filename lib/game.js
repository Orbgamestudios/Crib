...
export class Game {
  ...
  constructor(seats, hooks) {
    ...
    this.players = seats.map((s, i) => ({
      ...
      scoringDone: false, dealPoints: 0, shopOffer: null,
    }));
  ...
  playCard(p, cardId) {
    ...
    this.lastPlayAnim = { seq: ++this.animSeq, card: playedCard };
  ...
  openShop(preserveReady = false) {
    this.phase = 'shop';
    for (const p of this.players) {
      if (p.active) this.ensureShopOffer(p);
      else p.shopOffer = null;
