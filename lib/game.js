...
function rubyDeckCard(c, suits) {
  ...
  return makeCard(c.rank, suits[Math.floor(Math.random() * suits.length)]);
}
...
export class Game {
  ...
  static fromSnapshot(snapshot, hooks) {
    ...
    for (const p of game.players) {
      ...
      if (typeof p.gambitHandBonus !== 'number') p.gambitHandBonus = 0;
    }
  ...
  startDeal() {
    ...
    if (k === 3) this.crib.push(this.mode === 'board' ? this.sharedDrawPile.pop() : this.bySeat(this.dealerSeat).drawPile.pop());
    this.phase = 'discard';
  ...
  discard(p, cardIds) {
    ...
    const cribCards = pmods.cribFives ? cribPicks.map(c => makeCard(5, c.suit)) : cribPicks;
    ...
    if (pmods.cribTenTarot && p.tarots.length < this.maxTarots(p)) {
      const worth = cribPicks.reduce((s, c) => s + cardValue(c.rank), 0);
      if (worth >= 10) {
        const t = randomTarot();
        p.tarots.push(t.id);
  ...
  doScoring() {
    ...
    for (let i = 1; i <= n; i++) {
      ...
      if (this.deckIs(p, 'gambit') && p.gambitHandBonus > 0) {
        handPoints += p.gambitHandBonus;
        lines.push({ label: `Gambit charge [Deck]`, pts: p.gambitHandBonus });
      }
      handBlocks.push({
  ...
  stateFor(playerId) {
    ...
    return {
      ...
      you: me ? {
        ...
        deckEffects: this.deckEffects,
        gambitHandBonus: me.gambitHandBonus || 0,
