...
export class Game {
  ...
  doScoring() {
    ...
    const cribMult = dmods.cribScoreMult +
      Math.floor(this.crib.length / 2) * dmods.cribPairMult +
      cribBd.fifteens * dmods.cribFifteenMult;
    if (cribMult > 1) cribRes.lines.push({ label: `Crib Mult x${cribMult} [Joker]`, pts: null });
    // the crib scores straight points unless crib jokers grant Mult
    const cribBlock = {
      kind: 'crib', player: dealer, seat: dealer.seat, name: dealer.name,
      cards: this.crib.slice(), lines: cribRes.lines, points: cribRes.total, mult: cribMult, noMult: cribMult <= 1,
    };
