...
export class Game {
  ...
  useTarot(p, tarotIdx, targetIds) {
    ...
    const shopUse = this.phase === 'shop' && !p.ready && def.targets === 0 && def.id !== 'wheel';
  ...
  enterRoundEnd() {
    ...
    passers.forEach((r, place) => {
      const p = this.bySeat(r.seat);
      r.place = place + 1;
      r.bonusCoins = Math.max(1, 5 - place); // 1st:5, 2nd:4, 3rd:3, 4th:2, rest:1
  ...
  sellJoker(p, idx) {
    if (this.phase !== 'shop') return 'You can only sell in the shop.';
    const id = p.jokers[idx];
    const def = id && JOKERS_BY_ID[id];
