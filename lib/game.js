...
import {
  TAROTS_BY_ID,
  aggregateMods, buildScore, buildScoreOrdered, makeShopOffer, openPack, randomJoker, randomTarot,
...
const DECK_IDS = new Set(['classic', 'emerald', 'sapphire', 'ruby', 'aurora', 'neon', 'cosmic']);
...
function redDeckCard(c) {
  if (!c) return c;
  if (c.suit === 3) return makeCard(c.rank, 0);
  if (c.suit === 2) return makeCard(c.rank, 1);
  return c;
}
...
export class Game {
  ...
  constructor(seats, hooks, opts = {}) {
    ...
    this.players = seats.map((s, i) => ({
      ...
      deck: makeDeck(), drawPile: [], pendingPack: null, pegPlays: 0, dealPegMult: 0, cosmicHandBonus: 0,
  ...
  deckIs(p, id) { return normalizeDeckArt(p && p.deckArt) === id; }
  mods(p) { return aggregateMods(p.jokers); }
  maxJokers(p) { return jokerCapacity(p.jokers, MAX_JOKERS + (this.deckIs(p, 'sapphire') ? 1 : 0) - (this.deckIs(p, 'neon') ? 1 : 0)); }
  maxTarots(p) { return Math.max(0, MAX_TAROTS - (this.deckIs(p, 'neon') ? 1 : 0)); }
    ...
    for (const raw of effectiveJokerIds(p.jokers)) {
      ...
      switch (def.id) {
        ...
        case 'crib_spark':
          ...
          break;
        case 'crib_battery': {
          const suits = new Set(this.crib.map(c => c.suit)).size;
          if (suits) {
  ...
  startDeal() {
    ...
    for (const p of this.players) {
      if (p.active) {
        if (this.mode !== 'board' && this.deckIs(p, 'ruby')) p.deck = p.deck.map(redDeckCard);
        const pile = this.mode === 'board' ? boardPile : shuffle(p.deck.slice());
        const drawCount = this.cardsEach + (this.mode !== 'board' && this.deckIs(p, 'aurora') ? 1 : 0);
      ...
      p.dealPegMult = this.mode === 'board' ? 0 : (this.mods(p).stampRedMult || 0);
      p.rerollCost = REROLL_START_COST;
      p.discarded = !p.active; p.ready = false; p.scoringDone = false; p.dealPoints = 0; p.cosmicHandBonus = 0;
      p.coinGain = 0;
      if (this.mode === 'board') {
        p.jokers = [];
        p.tarots = [];
    ...
    const auroraCrib = this.mode !== 'board' && this.actives().some(p => this.deckIs(p, 'aurora'));
    if (k === 3 && !auroraCrib) this.crib.push(this.mode === 'board' ? this.sharedDrawPile.pop() : this.bySeat(this.dealerSeat).drawPile.pop());
    this.phase = 'discard';
  ...
  discard(p, cardIds) {
    ...
    const cribCards = pmods.cribFives ? picked.map(c => makeCard(5, c.suit)) : picked;
    ...
    p.discarded = true;
    this.log(`${p.name} sent ${need} card(s) to the crib`);
    this.touch();
  ...
  playCard(p, cardId) {
    ...
    if (this.mode !== 'board' && mods.smallBall && card.rank >= 2 && card.rank <= 5) {
    ...
    if (this.mode !== 'board' && this.deckIs(p, 'cosmic') && events.some(ev => ev.type === 'fifteen')) {
      pts += 2;
      p.cosmicHandBonus = (p.cosmicHandBonus || 0) + 2;
      this.log(`  Cosmic Drift target +2 Hand, +2 Mult`);
    }
    p.pegPlays++;
  ...
  doScoring() {
    ...
    for (let i = 1; i <= n; i++) {
      ...
      const cosmicPegBonus = this.mode !== 'board' && this.deckIs(p, 'cosmic') ? (p.cosmicHandBonus || 0) : 0;
      if (cosmicPegBonus) {
        handPoints += cosmicPegBonus;
        lines.push({ label: 'Cosmic Drift pegging targets', pts: cosmicPegBonus });
      }
      if (this.mode !== 'board' && this.deckIs(p, 'cosmic') && bd.fifteens) {
        handPoints += 2;
        handMult += 2;
        lines.push({ label: `Cosmic Drift target ${target}`, pts: 2 });
      }
      handBlocks.push({
    ...
    if (this.mode !== 'board' && this.deckIs(dealer, 'cosmic') && cribBd.fifteens) {
      cribPoints += 2;
      cribMult += 2;
      cribRes.lines.push({ label: `Cosmic Drift target ${cribTarget}`, pts: 2 });
    }
    const cribBlock = {
    ...
    if (this.mode !== 'board') for (const p of this.actives()) {
      ...
      const interest = this.deckIs(p, 'emerald') ? Math.min(3, Math.floor(p.coins / 10)) : 0;
      const gain = base + mods.coinsPerDeal + mods.coinPerBlind * p.blindsPassed + interest;
      p.coins += gain;
      p.coinGain = gain;
    }
  ...
  reorderJokers(p, order) {
    ...
    const key = j => {
      ...
      return owned ? `${owned.id}:${owned.stamp || ''}` : '';
    };
  ...
  stateFor(playerId) {
    ...
    return {
      ...
      pegCount: this.pegCount,
        ...
        cribCards: cosmicSight ? this.crib.slice() : null,
        mySeat: me ? me.seat : -1,
      ...
      you: me ? {
        ...
        blindsPassed: me.blindsPassed, dealMult: BASE_MULT + me.dealPegMult, cosmicHandBonus: me.cosmicHandBonus || 0,
        ...
        jokers: this.mode === 'board' ? [] : me.jokers.map(j => ({ ...jokerDef(j), stamp: normalizeJoker(j)?.stamp })),
