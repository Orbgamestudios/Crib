...
// Each deal scores hand Points × pegging Mult (Balatro-style), so per-deal
...
const BASE_MULT = 2;        // every hand starts at ×2; pegging adds to it
const PER_DEAL_YIELD = 34;  // avg hand Points × avg pegging Mult per deal
const PER_CRIB_YIELD = 18;  // avg crib Points × dealer's Mult
...
export function computeBlind(round, playerCount, dealsInRound) {
  ...
  const mult = 0.30 * Math.pow(1.33, round - 1) * HARDER;
  return Math.max(20, Math.round(expected * mult / 5) * 5);
}
...
export class Game {
  ...
  botTick() {
    try {
      ...
      for (const p of this.players) {
        ...
        else if ((this.phase === 'scoring' || this.phase === 'roundEnd') && !p.ready) {
  ...
  discard(p, cardIds) {
    ...
    const copies = this.mods(p).duplicateFirstCrib;
    if (copies && picked[0]) {
      for (let i = 0; i < copies; i++) this.crib.push(makeCard(picked[0].rank, picked[0].suit));
      this.log(`${p.name}'s Crib Copier copied ${cardName(picked[0])} into the crib`);
    }
    p.discarded = true;
  ...
  advancePeg(justPlayedSeat) {
    ...
    if (!this.last31) {
      const goPts = this.mods(last).goVal * this.mods(last).pegMult;
  ...
  doScoring() {
    ...
    for (const b of handBlocks) {
      ...
      for (const other of handBlocks) {
        ...
        b.points += mug;
        b.lines.push({ label: `Muggins on ${other.name} 🃏`, pts: mug });
      }
    }
    ...
    const cribBlock = {
      ...
      cards: this.crib.slice(), lines: cribRes.lines, points: cribRes.total, mult: BASE_MULT + dealer.dealPegMult,
    };
    ...
    const finalize = b => {
      ...
      results.push({
        ...
        lines: b.lines, points: b.points, mult: b.mult, total: b.total, starter,
      });
    };
    ...
    this.revealTimer = setInterval(() => {
      if (this.revealIndex < this.scoringResults.length - 1) {
        ...
        if (this.revealIndex >= this.scoringResults.length - 1) {
          ...
          setTimeout(() => {
            this.advanceFromScoring(); // auto-open the shop — nobody waits to shop
            this.onUpdate();
          }, SCORE_DONE_HOLD_MS);
          this.onUpdate();
        }
  ...
  enterRoundEnd() {
    ...
    for (const r of rows) {
      ...
      if (r.passed) {
        p.blindsPassed++;
        r.bonusCoins = this.overBlindBonus(r.roundScore);
        if (r.bonusCoins > 0) {
          p.coins += r.bonusCoins;
          p.coinGain = (p.coinGain || 0) + r.bonusCoins;
          this.log(`${p.name} earned ${r.bonusCoins} bonus coin(s) for beating the blind by ${r.roundScore - this.blind}.`);
        }
      } else {
        r.bonusCoins = 0;
        p.active = false;
  ...
  overBlindBonus(roundScore) {
    if (roundScore <= this.blind) return 0;
    return Math.min(6, Math.floor((roundScore - this.blind) / Math.max(12, Math.ceil(this.blind * 0.25))));
  }
  ...
  setReady(p) {
    ...
    if (!p.active) return;
    if (p.pendingPack) return 'Open your booster pack first.';
    // Scoring is not a sync gate: any player can jump to the shop early, and
    // otherwise the reveal auto-advances. The shop is the next-deal sync point.
    if (this.phase === 'scoring') { this.advanceFromScoring(); this.onUpdate(); return; }
    p.ready = true;
    this.touch();
    this.maybeAdvance();
    this.onUpdate();
