...
export class Game {
    ...
    for (const def of this.effectiveJokers(p)) {
      switch (def.id) {
        ...
        case 'card_sharp':
          ...
          break;
        case 'onyx_agate':
          if (events.length && card.suit === 2) {
            total += 2;
            this.log('  Onyx Agate +2 Mult [Joker]');
          }
          break;
        case 'bloodstone':
          if (events.length && card.suit === 0 && Math.random() < 0.5) {
            total *= 1.5;
  ...
  scoreFinalMultOrdered(p, base, kind, cards, starter) {
    ...
    const lines = [];
    const ranks = new Map();
    for (const card of starter ? cards.concat(starter) : cards) ranks.set(card.rank, (ranks.get(card.rank) || 0) + 1);
    for (const raw of effectiveJokerIds(p.jokers)) {
      const owned = normalizeJoker(raw);
      const def = owned && jokerDef(owned);
      if (!def) continue;
  ...
  playCard(p, cardId) {
    ...
    const roughGems = this.mode === 'board' || !events.length || card.suit !== 1
      ? 0
